...
const CategoriesTab = ({
  ...
  <div className="space-y-3 mt-3">
    ...
    {categories.map((cat, idx) => (
      <div
        key={cat.id}
        /* min-w-0 allows the container to shrink smaller than its content, enabling truncation */
      ...
      >
        ...
        {editingCat?.id === cat.id ? (
          <>
            ...
            />
            <Button size="sm" variant="ghost" onClick={onSaveEditCat} className="shrink-0 h-8">
              Save
            </Button>
          ...
          <>
            {/* truncate + min-w-0 + flex-1 forces the text to stay on one line with dots (...) */}
            <span 
              className="flex-1 text-sm text-foreground font-sans truncate min-w-0" 
              title={cat.name}
            >
              {cat.name}
            </span>
