...
const CategoriesTab = ({
  categories, catName, onCatNameChange, onAddCategory,
  editingCat, onSetEditingCat, onSaveEditCat, onDeleteRequest,
      ...
      >
        ...
        {editingCat?.id === cat.id ? (
          <>
            ...
            />
            <Button size="sm" variant="ghost" onClick={onSaveEditCat}>Save</Button>
            <Button size="sm" variant="ghost" onClick={() => onSetEditingCat(null)}>
              <X className="w-3 h-3" />
            </Button>
          </>
        ) : (
          <>
            <span className="flex-1 text-sm text-foreground font-sans truncate" title={cat.name}>
              {cat.name}
            </span>
            <Button size="sm" variant="ghost" className="shrink-0" onClick={() => onSetEditingCat(cat)}>
