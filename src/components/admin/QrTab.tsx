...
// Common utility for toast positioning
const notify = {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (!useFull && restaurant.logo_url) {
        ...
        if (!logoData) {
          notify.error("Logo load failed. Generating full QR code...");
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      qrImg.onerror = () => {
        if (qrUrl) URL.revokeObjectURL(qrUrl);
        setIsSharing(false);
        notify.error("Failed to generate QR image");
      };
      qrImg.src = qrUrl;

    } catch (e: any) {
      if (qrUrl) URL.revokeObjectURL(qrUrl);
      if (e.name !== 'AbortError') notify.error("Share failed");
      setIsSharing(false);
    }
  };
