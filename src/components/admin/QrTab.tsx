...
const QrTab = ({ restaurant, menuUrl, onViewFullscreen }: Props) => {
  ...
  // Listen for failure messages from the print window
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      if (e.data === 'fail') {
        toast.error("Logo failed for print. Using full QR.", { position: "top-center" });
      }
    };
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      toast.error("Could not open print window. Check if popups are blocked.");
    } finally {
      setIsPrinting(false);
    }
  };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const svgString = new XMLSerializer().serializeToString(svgToUse?.querySelector("svg")!);
